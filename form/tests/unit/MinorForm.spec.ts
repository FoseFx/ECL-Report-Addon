...
describe('MinorForm.vue', () => {
  ...
  it('it should select right submit function', async () => {
    ...
    wrapper.setData({where: 'sthelse', valid: true, offenceInvalid: false});
    // @ts-ignore
    ...
    // proof invalid
    wrapper.setData({where: 'in-game', valid: true, offenceInvalid: false});
    wrapper.vm.$refs.proofs = [{valid: () => false} as unknown as ProofComponent];
    // @ts-ignore
    expect(await wrapper.vm.onSubmit()).toEqual(false);
  ...
  it('should evalLinks', async () => {
    ...
    imgur.uploadToImgur = jest.fn()
      ...
      .mockRejectedValueOnce(null); // upload fails
    ...
    // @ts-ignore
    expect(await wrapper.vm.evalLinks()).toEqual(null);
  ...
  it('should submit ingame', async () => {
    ...
    // @ts-ignore
    const evalLinksSpy = spyOn(wrapper.vm, 'evalLinks').and.returnValue(
      Promise.resolve({
        proofLink: 'first url',
        additionalLinksData: [
          {link: 'snd url'},
    ...
    // eval failed
    evalLinksSpy.and.returnValue(Promise.resolve(null));
    // @ts-ignore
    expect(await wrapper.vm.onSubmitInGame()).toEqual(false);
  ...
  it('should submit matchroom', async () => {
    ...
    evalLinksSpy.and.returnValue(Promise.resolve(null));
    // @ts-ignore
    expect(await wrapper.vm.onSubmitMatchroom()).toEqual(false);
    expect(wrapper.emitted('submitted').length).toEqual(1); // no new event
  });
});
